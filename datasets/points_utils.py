...
def getlabelPC_train(PC, maxi, mini):
    """
    ...
    a, b, c = PC.size()
    maxi = maxi.unsqueeze(1).repeat(1, 64, 1)
    mini = mini.unsqueeze(1).repeat(1, 64, 1)
    # PC = PC.cpu().numpy()
    # maxi = maxi.cpu().numpy()
    # mini = mini.cpu().numpy()
    ...
    close = torch.logical_and(x_filt_min, x_filt_max)
    close = torch.logical_and(close, y_filt_min)
    close = torch.logical_and(close, y_filt_max)
    close = torch.logical_and(close, z_filt_min)
    close = torch.logical_and(close, z_filt_max)

    new_label = torch.zeros((a,b))
    new_label[close] = 1
